...
class ProductionSpectralAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, fractal_dim: torch.Tensor) -> torch.Tensor:
        ...
        # Attention in frequency domain with proper complex handling
        # Robust dimension handling for any input shape
        q_real = q_filtered.real
        k_real = k_filtered.real
        v_real = v_filtered.real
        
        # Get original shapes for later reshaping
        original_shape = q_real.shape
        ...
        q_flat = q_real.reshape(-1, *q_real.shape[-2:])  # [batch*seq, heads, head_dim]
        k_flat = k_real.reshape(-1, *k_real.shape[-2:])  # [batch*seq, heads, head_dim]
        v_flat = v_real.reshape(-1, *v_real.shape[-2:])  # [batch*seq, heads, head_dim]
